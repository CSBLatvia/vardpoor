...
lingini2 <- function(inc, id = NULL, weight = NULL, sort = NULL, 
        Dom = NULL, dataset = NULL, na.rm = FALSE, var_name="lin_gini2") {
  ...
  # Gini by domain (if requested)
   ...
   if(!is.null(Dom)) {
      ...
      for(k in 1:nrow(Dom_agg)) {
              ...
              gini_l <- lingini2Calc(inc[ind,1],id[ind,1], weight[ind], sort[ind], na.rm=na.rm)
              ginilin <- gini_l$lin
     ...
     } else { gini_l <- lingini2Calc(inc[,1],id[,1], weight, sort, na.rm=na.rm)
              gini_m <- gini_l$lin
