...
linpoormed<- function(inc, id, weight=NULL, sort=NULL, Dom=NULL, dataset = NULL,
                     percentage=60, order_quant=50, na.rm=FALSE, var_name="lin_medpoor") {
   ...
   if(!is.null(Dom)) {
    ...
    if(!is.null(Dom)) {
       ...
       for(i in 1:nrow(Dom_agg)) {
              ...
              poormed_l <- linpoormedCalc(inc[,1], id, weight, ind=indicator, percentag=p,
                                      order_quan=order_quant, quant_val=quantiles, na.rm=na.rm) 
              poormedl <- poormed_l$lin
              colnames(poormedl) <- c(names(id),paste(var_name,breakdown2,sep="__"))
              poor_med_m <- merge(poor_med_m,poormedl,by=colnames(id),all.x=T)
              poor_med_m[is.na(poor_med_m[,ncol(poor_med_m)]),ncol(poor_med_m)] <- 0
       ...
       poor_people_median <- data.frame(Dom_agg,poor_people_median)
              ...
              colnames(poor_med_m) <- c(colnames(id),var_name)
              poor_people_median <- poormed_l$poor_people_median   }
...
## workhorse
linpoormedCalc <- function(inco, ids, wght, ind, percentag, order_quan, quant_val, na.rm) {
    if(isTRUE(na.rm)){
        indices <- !is.na(inco)
        ids <- ids[indices]
        inco <- inc1[indices]
