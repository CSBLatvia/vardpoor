...
check_var <- function(vars, varn, dataset, check.names = FALSE,
                      ...
                      mustdefined = TRUE, asvector = FALSE,
                      grepls = NULL, dif_name = "", namesID1 = "id",
                      duplicatednames = FALSE, withperiod = TRUE,
                      PSUs = NULL, periods = NULL, periods_varn  = NULL,
                      country = NULL, periodsX = NULL){
  ...
  if (!is.null(vars)) {
      ...
      if (names(vars) == dif_name) stop(paste0("'", varn, "' must be different name"), call. = FALSE)
    ...
    } else if (mustdefined) stop(paste0("'", varn, "' must be defined!"), call. = FALSE)
