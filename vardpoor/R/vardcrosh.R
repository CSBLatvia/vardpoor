#vardcrosh <- function(Y, H, PSU, w_final,
#                       ID_level1,
#                       ID_level2,
#                       Dom = NULL,
#                       Z = NULL, 
...
#                       confidence = .95) {
  ...
  if (!is.null(X)) {
     if(!is.null(periodX)) {
        ...
        peri <- data.table(unique(periods))
        ...
        if (ncol(periodX) != ncol(periods)) stop("'periodX' length must be equal with 'period' column count")
        if (names(periodX) != names(periods)) stop("'periodX' must be equal with 'period' names")
        if (any(is.na(periodX))) stop("'periodX' has missing values")
        if (any(peri != periX)) stop("'unique(periods)' and 'unique(periodX)' records have different")
        if (peri[, class(get(names(peri)))] != periX[, class(get(names(periX)))])  stop("Class for 'periodX' and class for 'periods' must be equal")
      } else if (!is.null(periods)) stop("'periodX' must be defined")
   } 
 ...
 # X_ID_level1
  if (!is.null(X)) {
    ...
    if (!is.null(periodX)) {X_ID_level1 <- data.table(periodX, X_ID_level1)
                            ID_level1h <- data.table(periods, ID_level1)
    ...
    if (!is.null(periods)) {
        if (nrow(ID_level1h) != nrow(X_ID_level1)) stop("'periodX' with 'X_ID_level1' and 'unique(periods, ID_level1)' have different row count")
        if (any(ID_level1h != X_ID_level1)) stop("'periodX' with 'X_ID_level1' and 'unique(periods, ID_level1)' records have different")
      } else {
        if (nrow(ID_level1h) != nrow(X_ID_level1)) stop("'X_ID_level1' and 'unique(ID_level1)' have different row count")
        if (any(ID_level1h != X_ID_level1)) stop("'X_ID_level1' and 'unique(ID_level1)' records have different")
    }}
  ...
  if (!is.null(X)) {
             ...
             if (!is.null(periods)) idh <- data.table(periods, idh)
  ...
  if (!is.null(Z)) {
   ...
   size1wn <- paste0(names_size1, "w")
 
   Y2w <- copy(Y2)
   setnames(Y2w, namesY2, namesY2w)

   size1w <- size1 * w_final
   setnames(size1w, names_size1, size1wn)
   ...
   DTc <- data.table(period_country, DTp, H, PSU, ID_level1, w_final,
                    , size1, size1w, Y2, Y2w) 
  ...
  DT2 <- DT1[, lapply(.SD, sum, na.rm = TRUE),
            key by = namesperc, .SDcols = namesY2]
  varsYZ <- list(namesY1)
 ...
 #--------------------------------------------------------------------------*
  ...
  DTs <- DT[, lapply(.SD, sum, na.rm = TRUE), 
                          key by = c(namesperc2, names_id, "w_final"),
                         .SDcols = c(names_size1, size1wn, namesY2)]
  if (household_level_max) {    
           DTm <- DT[, lapply(.SD, max, na.rm = TRUE), key by = c(namesperc2, names_id), .SDcols = names_size1]
       } else {
          DTm <- DT[, lapply(.SD, sum, na.rm = TRUE), key by = c(namesperc2, names_id), .SDcols = names_size1]
     }
  ...
  DTsd <- DTs[, lapply(.SD, sd, na.rm = TRUE), key by = "period_country", .SDcols = Y2a]
  setnames(DTsd, Y2a, paste0("sd_nw__", namesY1))
  DTs <- merge(DTs, DTsd,  by = "period_country")

  DTm <- DTs[, lapply(.SD, sum, na.rm = TRUE), key by = "period_country", .SDcols = names_size1]
  setnames(DTm, names_size1, paste0("samp_", names_size1))
  DTs <- merge(DTs, DTm,  by = "period_country")
    
  DTx <- DTs[, .N, key by = c(namesperc, paste0("sd_w__", namesY1),
