...
test_fun <- function(n1 = 3000){ 
  ...
  #vardom dati
  nn <- round(n1 / 300, 0) #number of PSU (iecirkni)
  dat_x <- data.table()
  strata <- sample(1:4, n1, replace = T,)
  dat_x <- data.table(id_m = 1:n1,
                      b06 = sample(1:11, n1, replace = T, prob = c(0.1922021, 0.1302067, 0.0641481, 0.0471424, 0.01797438,
                                                                   0.007695619, 0.002179529, 0.00131848, 0.0003498009, 0.000107631, 2.690776e-05)),
  ...
  n2 <- nrow(dat_y)
  if ((nrow(dat_y) - sum(dat_x$b06)) !=0) stop("nekorekti datu garumi")
  
  dat_y[, y := sample(0:1, n2, replace = T)]#piekārto y vērtību
  gg <- g_dat[, c("id_m", "g")]
  ...
  dat_x$y <- ycol$y # papildina vardom datni
  ...
  res_1 <- res_1[, -c("respondent_count", "n_nonzero","pop_size")] # #radītāji atšķirās, jo rēķina no dažādiem līmeņiem
...
#### 2 periodi, ar kalibrāciju, bez Z ----
...
test_fun2 <- function(n1){
  ...
  n_h <- data.table(period = rep(1:2, each = 4), strata = rep(1:4, 2), pop = rep(sample(500:1500, 4),2))
...
#res_1[, -c("variable", "respondent_count", "n_nonzero", "pop_size", "n_eff", "period")] - res_2[, -c("variable", "respondent_count", "n_nonzero", "pop_size", "period")]
